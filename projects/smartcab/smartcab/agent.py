...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        k = self.genKey(state)
        actions = pd.Series(self.Q[k])
        maxValue = max(self.Q[k].values())
    ...
    def genKey(self,state):
        #create this key as string doesn't warranty the order
        return (state[0],state[1]["light"],state[1]["oncoming"],state[1]["right"],state[1]["left"])


    def createQ(self, state):
        """ The createQ function is called when a state is generated by the agent. """
        if self.learning:
          key = self.genKey(state)
          if key in self.Q:
              return
          else:
              actions = {}
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
          k = self.genKey(state)
          actualValue = self.Q[k][action]
          self.Q[k][action] = (1-self.alpha)*self.Q[k][action]+reward*self.alpha
        ########### 
        ## TO DO ##
        ###########
